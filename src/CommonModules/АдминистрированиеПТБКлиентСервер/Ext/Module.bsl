...
Функция КомандаЗапускаСозданияБазы(знач ПутьКФайлуПлатформы, знач ПараметрыСоздания) Экспорт
	...
	Если ТипПараметры = Тип("Строка") Тогда
		Настройки = ПолучитьНастройкиСозданияБазы(0);
	...
	Иначе 
		ВызватьИсключение НСтр("ru='Неверно передан ""ПараметрыСоздания"" метода ""АдминистрированиеАСУПКлиентСервер.КомандаЗапускаСозданияБазы"".'") ;
	КонецЕсли;
...
Функция КомандаЗапускаКонфигуратора(знач ПутьКФайлуПлатформы, знач ПараметрыПодключенияКБазе, знач ОтключитьЗаставку = Истина) Экспорт
	Возврат СтрШаблон("%1 DESIGNER %2 /DisableStartupMessages%3",
		ПутьКФайлуПлатформы,
...
// Возвращает имя исполняемого файла 1cv8.exe или 1cv8s.exe
//
// Параметры:
//	ЗапускНаКлиенте - Булево - признак запуска на клиенте
// Истина - 1cv8s
...
//		Вид клиента определяется из настроек в списке баз
// Ложь - 1cv8
...
//   Строка
// 
Функция ИмяИсполняемогоФайлаПлатформы(знач ЗапускНаКлиенте = Ложь) Экспорт 
	Если ЗапускНаКлиенте = Истина Тогда
		Возврат "1cv8s.exe";
	КонецЕсли;
...
Функция ПараметрыОбновленияИзФайла(знач Настройки) Экспорт
	...
	ОбновлятьКонфигурациюБД			= ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Настройки, "ОбновлятьКонфигурациюБД", Истина);
	...
	Возврат КомандаОбновления + ?(ОбновлятьКонфигурациюБД, " /UpdateDBCfg", "");
КонецФункции
...
Функция ПараметрыВыгрузкиКонфигурацииВФайлы(знач Настройки) Экспорт
	...
	Если Настройки.Свойство("ВсеРасширения") И Настройки.ВсеРасширения Тогда
		СтрПараметры = СтрПараметры + " –AllExtensions";
	ИначеЕсли Настройки.Свойство("ИмяРасширения") И НЕ ПустаяСтрока(Настройки.ИмяРасширения) Тогда
		СтрПараметры = СтрПараметры + " -Extension " + Настройки.ИмяРасширения;
	КонецЕсли;
	
	Если Настройки.Свойство("ВыгружатьИзменения") И Настройки.ВыгружатьИзменения Тогда
		СтрПараметры = СтрПараметры + " -update -force";
		
		Если НЕ ПустаяСтрока(Настройки.ФайлИзменений) Тогда
			СтрПараметры = СтрПараметры + "	-getChanges ""%ФайлИзменений%""";
			СтрПараметры = СтрЗаменить(СтрПараметры, "%ФайлИзменений%", Настройки.ФайлИзменений);
		КонецЕсли;
		
		Если НЕ ПустаяСтрока(Настройки.ФайлПредыдущейВерсии) Тогда
			СтрПараметры = СтрПараметры + " -configDumpInfoForChanges ""%ФайлПредыдущейВерсии%""";
			СтрПараметры = СтрЗаменить(СтрПараметры, "%ФайлПредыдущейВерсии%", Настройки.ФайлПредыдущейВерсии);
		КонецЕсли;
	ИначеЕсли Настройки.Свойство("ВыгружатьПоСписку") И Настройки.ВыгружатьПоСписку Тогда
		СтрПараметры = " -listFile ""%ФайлСпискаВыгрузки%""";
		СтрПараметры = СтрЗаменить(СтрПараметры, "%ФайлСпискаВыгрузки%", Настройки.ФайлСпискаВыгрузки);
	КонецЕсли;
	
	Если Настройки.Свойство("ТолькоConfigDumpInfo") И Настройки.ТолькоConfigDumpInfo Тогда
		СтрПараметры = СтрПараметры + " -configDumpInfoOnly";
	КонецЕсли;
	
	Если Настройки.Свойство("Иерархически") И Настройки.Иерархически = Ложь Тогда
...
Функция ПараметрыЗагрузкиКонфигурацииИзФайлов(знач Настройки) Экспорт
	...
	Если Настройки.Свойство("ФайлСпискаЗагрузки") И НЕ ПустаяСтрока(Настройки.ФайлСпискаЗагрузки) Тогда
		СтрПараметры = СтрПараметры + " -listFile ""%ФайлСпискаЗагрузки%""";
		СтрПараметры = СтрЗаменить(СтрПараметры, "%ФайлСпискаЗагрузки%", Настройки.ФайлСпискаЗагрузки);
	КонецЕсли;
	...
	Если Настройки.Свойство("ОбновитьФайлВерсии") И Настройки.ОбновитьФайлВерсии = Истина Тогда
		СтрОбновитьФайлВерсии = " -updateConfigDumpInfo";
	Иначе
		СтрОбновитьФайлВерсии = "";
	КонецЕсли;
	
	Если Настройки.Свойство("ОбновлятьКонфигурациюБД") И Настройки.ОбновлятьКонфигурациюБД = Истина Тогда
		СтрОбновитьКонфигурациюБД = " /UpdateDBCfg";
	Иначе
		СтрОбновитьКонфигурациюБД = "";
	КонецЕсли;
	
	Возврат СтрШаблон("/LoadConfigFromFiles ""%1""%2%3%4%5",
		Настройки.КаталогЗагрузки,
		СтрФормат,
		СтрПараметры,
		СтрОбновитьФайлВерсии,
		СтрОбновитьКонфигурациюБД);
...
Функция ПараметрыСозданияДистрибутива(знач Настройки) Экспорт
	ПутьКФайлуCF			= Настройки.ПутьКФайлуCF;
	ПутьКФайлуCFU			= Настройки.ПутьКФайлуCFU;
	Основания				= Настройки.Основания;
	
	Если НЕ ЗначениеЗаполнено(ПутьКФайлуCF) И НЕ ЗначениеЗаполнено(ПутьКФайлуCFU) Тогда
		Возврат "";
	КонецЕсли;
...
//
// Параметры:
//	Настройки - Структура - см. ПолучитьНастройкиСозданияБазы
//
// Возвращаемое значение:
//   Строка
// 
Функция ПараметрыСозданияБазы(знач Настройки) Экспорт
	Если Настройки.ТипБД = 1 Тогда
...
Функция ПараметрыПроверкиМодулей(знач Настройки) Экспорт
	...
	Если Настройки.Свойство("ТонкийКлиент") И Настройки.ТонкийКлиент = Истина Тогда
		СтрПараметры = СтрПараметры + " –ThinClient";
	КонецЕсли;
	
	Если Настройки.Свойство("ВебКлиент") И Настройки.ВебКлиент = Истина Тогда
		СтрПараметры = СтрПараметры + " –WebClient";
	КонецЕсли;
	
	Если Настройки.Свойство("МобильныйКлиент") И Настройки.МобильныйКлиент = Истина Тогда
		СтрПараметры = СтрПараметры + " –MobileClient";
	КонецЕсли;
	
	Если Настройки.Свойство("КлиентМобильногоПриложения") И Настройки.КлиентМобильногоПриложения = Истина Тогда
		СтрПараметры = СтрПараметры + " –MobileAppClient";
	КонецЕсли;
	
	Если Настройки.Свойство("Сервер") И Настройки.Сервер = Истина Тогда
		СтрПараметры = СтрПараметры + " –Server";
	КонецЕсли;
	
	Если Настройки.Свойство("СерверМобильногоПриложения") И Настройки.СерверМобильногоПриложения = Истина Тогда
		СтрПараметры = СтрПараметры + " –MobileAppServer";
	КонецЕсли;
	
	Если Настройки.Свойство("ВнешнееСоединение") И Настройки.ВнешнееСоединение = Истина Тогда
		СтрПараметры = СтрПараметры + " –ExternalConnection";
	КонецЕсли;
	
	Если Настройки.Свойство("ТолстыйКлиент") И Настройки.ТолстыйКлиент = Истина Тогда
		СтрПараметры = СтрПараметры + " –ThickClientOrdinaryApplication";
	КонецЕсли;
	
	Если Настройки.Свойство("РасширеннаяПроверка") И Настройки.РасширеннаяПроверка = Истина Тогда
...
Функция ПараметрыПроверкиКонфигурации(знач Настройки) Экспорт
	...
	Если Настройки.Свойство("ПроверкаЛогическойЦелостностиКонфигурации") И Настройки.ПроверкаЛогическойЦелостностиКонфигурации = Истина Тогда
		СтрПараметры = СтрПараметры + " –ConfigLogIntegrity";
	КонецЕсли;
	
	Если Настройки.Свойство("ПоискНекорректныхСсылок") И Настройки.ПоискНекорректныхСсылок = Истина Тогда
	...
	Если Настройки.Свойство("ТолстыйКлиентКлиентСервер") И Настройки.ТолстыйКлиентКлиентСервер = Истина Тогда
		СтрПараметры = СтрПараметры + " –ThickClientServerManagedApplication";
	КонецЕсли;
	
	Если Настройки.Свойство("МобильноеПриложениеКлиент") И Настройки.МобильноеПриложениеКлиент = Истина Тогда
		СтрПараметры = СтрПараметры + " –MobileAppClient";
	КонецЕсли;
	
	Если Настройки.Свойство("МобильноеПриложениеСервер") И Настройки.МобильноеПриложениеСервер = Истина Тогда
		СтрПараметры = СтрПараметры + " –MobileAppServer";
	КонецЕсли;
	...
	Если Настройки.Свойство("ВнешнееСоединениеКлиентСервер") И Настройки.ВнешнееСоединениеКлиентСервер = Истина Тогда
	...
	Если Настройки.Свойство("МобильныйКлиентАвтономныйРежим") И Настройки.МобильныйКлиентАвтономныйРежим = Истина Тогда
		СтрПараметры = СтрПараметры + " –MobileClientStandalone";
	КонецЕсли;
	
	Если Настройки.Свойство("ПоставкаБезИсходныхКодов") И Настройки.ПоставкаБезИсходныхКодов = Истина Тогда
		СтрПараметры = СтрПараметры + " –DistributiveModules";
	КонецЕсли;
	
	Если Настройки.Свойство("ТолстыйКлиентОбычноеПриложение") И Настройки.ТолстыйКлиентОбычноеПриложение = Истина Тогда
		СтрПараметры = СтрПараметры + " –ThickClientOrdinaryApplication";
	КонецЕсли;
	
	Если Настройки.Свойство("ТолстыйКлиентОбычнойПриложениеКлиентСервер") И Настройки.ТолстыйКлиентОбычнойПриложениеКлиентСервер = Истина Тогда
		СтрПараметры = СтрПараметры + " –ThickClientServerOrdinaryApplication";
	КонецЕсли;
	
	Если Настройки.Свойство("ПоискНеиспользуемыхМетодов") И Настройки.ПоискНеиспользуемыхМетодов = Истина Тогда
		СтрПараметры = СтрПараметры + " –UnreferenceProcedures";
	КонецЕсли;
	
	Если Настройки.Свойство("ПроверкаСуществованияНазначенныхОбработчиков") И Настройки.ПроверкаСуществованияНазначенныхОбработчиков = Истина Тогда
		СтрПараметры = СтрПараметры + " –HandlersExistence";
	КонецЕсли;
	
	Если Настройки.Свойство("ПоискПустыхОбработчиков") И Настройки.ПоискПустыхОбработчиков = Истина Тогда
	...
	Если Настройки.Свойство("РасширеннаяПроверка") И Настройки.РасширеннаяПроверка = Истина Тогда
		...
		Если Настройки.Свойство("ПоискИспользованияМодальности") И Настройки.ПоискИспользованияМодальности = Истина Тогда
			СтрПараметры = СтрПараметры + " –CheckUseModality";
		КонецЕсли;
		
		Если Настройки.Свойство("ПоискИспользованияСинхронныхВызовов") И Настройки.ПоискИспользованияСинхронныхВызовов = Истина Тогда
			СтрПараметры = СтрПараметры + " –CheckUseSynchronousCalls";
		КонецЕсли;
	КонецЕсли;
	
	Если Настройки.Свойство("ПоискНеподдерживаемойФункциональности") И Настройки.ПоискНеподдерживаемойФункциональности = Истина Тогда
		СтрПараметры = СтрПараметры + " –UnsupportedFunctional";
	КонецЕсли;
	
	Если Настройки.Свойство("ПроверкаПодписи") И Настройки.ПроверкаПодписи = Истина Тогда
...
//				Пароль	- Строка
// 
Функция ПолучитьНастройкиСозданияБазы(знач ТипБД) Экспорт 
	Настройки = Новый Структура;
...
Функция ПараметрыСозданияБазы_Серверная(знач Настройки)
	...
	Если НЕ ПустаяСтрока(ЗначDBUID_Pwd.Имя) Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Если НРег(Настройки.ТипСервераБД) = "mssqlserver" И ЗначSQLYOffs <> 2000 Тогда
