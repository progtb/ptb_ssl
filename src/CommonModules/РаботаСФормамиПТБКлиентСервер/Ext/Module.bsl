...
Процедура РассчитатьЗначенияВышестоящихЭлементов(ЭлементДерева, знач ИменаКолонок, знач ВидОперации = "СУММА", знач ВычислитьТекущуюСтроку = Ложь, знач МодульОповещения = Неопределено, знач ИмяПроцедуры = "") Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		#Если Клиент Тогда
			МодульОповещения	= Новый ОписаниеОповещения(ИмяПроцедуры, МодульОповещения);
		#Иначе
			МодульОповещения	= Неопределено;
			ИмяПроцедуры		= Неопределено;
		#КонецЕсли 
	КонецЕсли;
	
	РассчитатьЗначенияВышестоящихЭлементов_Рекурсивно(ЭлементДерева, МассивКолонок, ВидОперации, ВычислитьТекущуюСтроку, МодульОповещения, ИмяПроцедуры);
КонецПроцедуры
...
Процедура РассчитатьЗначенияВложенныхЭлементов(ЭлементДерева, знач ИменаКолонок, знач ВидОперации = "СУММА", знач МодульОповещения = Неопределено, знач ИмяПроцедуры = "") Экспорт
	...
	РассчитатьЗначенияВложенныхЭлементов_Рекурсивно(ЭлементДерева, МассивКолонок, ВидОперации, МодульОповещения, ИмяПроцедуры);
КонецПроцедуры
...
Процедура РассчитатьЗначенияВышестоящихЭлементов_Рекурсивно(СтрокаДерева, знач Колонки, знач Операция, знач ТекСтрока, знач Модуль, знач ИмяПроцедуры)
	...
	Если Модуль <> Неопределено Тогда
		#Если ВебКлиент Тогда
			ВыполнитьОбработкуОповещения(Модуль, СтрокаДерева);
		#Иначе
			Попытка
				Выполнить Модуль + "." + ИмяПроцедуры + "(СтрокаДерева)";
			Исключение
			КонецПопытки;
		#КонецЕсли 
	КонецЕсли;
	
	РассчитатьЗначенияВышестоящихЭлементов_Рекурсивно(ЭлементРодитель, Колонки, Операция, Ложь, Модуль, ИмяПроцедуры);
КонецПроцедуры

Процедура РассчитатьЗначенияВложенныхЭлементов_Рекурсивно(СтрокаДерева, знач Колонки, знач Операция, знач Модуль, знач ИмяПроцедуры)
	ВрегВидОперации = ВРег(Операция);
	...
	Для Каждого ВложенныйЭлемент Из НаборЭлементов Цикл
		РассчитатьЗначенияВложенныхЭлементов_Рекурсивно(ВложенныйЭлемент, Колонки, Операция, Модуль, ИмяПроцедуры);
	...
	Если ЭтоСтрокаДанныеФормы(СтрокаДерева, 1) Тогда
		Возврат;
	ИначеЕсли ЭтоСтрокаДереваЗначений(СтрокаДерева, 1) Тогда
		Возврат;
	КонецЕсли;
