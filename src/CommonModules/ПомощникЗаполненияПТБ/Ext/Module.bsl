...
// 
Функция ПолучитьТаблицуЗаполнения(Форма, знач ИмяРеквизитаКомпоновщик, знач Поля, знач ПараметрыДанных = Неопределено, знач Отбор = Неопределено, знач Порядок = Неопределено, знач Ресурсы = Неопределено, знач ВнешниеНаборыДанных = Неопределено) Экспорт
	...
	Для Каждого ИмяРесурса Из Ресурсы Цикл
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьВыбранноеПоле(НастройкиКомпоновки, ИмяРесурса);
	КонецЦикла;
	...
	// Таблица данных
	Возврат ОбщегоНазначенияПТБСервер.ПолучитьТаблицуЗначенийПоДаннымСКД(СхемаКомпоновкиДанных, КомпоновщикНастроек, ВнешниеНаборыДанных); 
...
Процедура ПроверитьСуществованиеРеквизитовПомощникаЗаполнения(Форма, знач ИмяРеквизитаКомпоновщик)
	...
	Попытка
		ЗначРеквизит1 = Форма[Реквизит1];
		ЕстьРеквизит1 = Истина;
	Исключение
		ЕстьРеквизит1 = Ложь;
	КонецПопытки;
	
	Попытка
		ЗначРеквизит2 = Форма[Реквизит2];
		ЕстьРеквизит2 = Истина;
	Исключение
		ЕстьРеквизит2 = Ложь;
	КонецПопытки;
