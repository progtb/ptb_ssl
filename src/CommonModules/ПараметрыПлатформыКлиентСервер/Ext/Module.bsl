...
Функция ПараметрыЗагрузкиКонфигурацииИзФайлов(знач Настройки) Экспорт
	...
	ОбновлятьКонфигурациюБД	= ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Настройки, "ОбновлятьКонфигурациюБД", Ложь);
	НастройкиОбновленияБД	= ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Настройки, "НастройкиОбновленияБД", Ложь);
	НеПроверятьЦелостность	= ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Настройки, "НеПроверять", Ложь);
	Иерархически			= ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Настройки, "Иерархически", Истина);
	...
	Если НеПроверятьЦелостность = Истина Тогда
		СтрПараметры = СтрПараметры + " -NoCheck";
	КонецЕсли;
...
// Возвращает строку для удаления расширения (или всех расширений)
//	/DeleteCfg
//
// Параметры:
//	ВсеРасширения	- Булево
//	ИмяРасширения	- Строка - обязательно, если ВсеРасширения = ЛОЖЬ
//
// Возвращаемое значение:
//   Строка
//
Функция ПараметрыУдаленияРасширения(знач ВсеРасширения = Истина, знач ИмяРасширения = "") Экспорт
	СтрПараметры = "";
	
	Если ВсеРасширения = Истина Тогда
		СтрПараметры = "-AllExtensions";
	ИначеЕсли ЗначениеЗаполнено(ИмяРасширения) Тогда
		СтрПараметры = СтрШаблон("-Extension %1", ИмяРасширения);
	Иначе 
		Возврат "";
	КонецЕсли;
	
	Возврат СтрШаблон("/DeleteCfg %1", СтрПараметры);
КонецФункции
...
Функция НастройкиЗагрузкиИзФайлов(знач КаталогЗагрузки = "", знач ВидЗагрузки = 0) Экспорт
	...
	Если ВидЗагрузки = 0 Тогда
		Настройки.Вставить("НеПроверять", Ложь);
	КонецЕсли;
