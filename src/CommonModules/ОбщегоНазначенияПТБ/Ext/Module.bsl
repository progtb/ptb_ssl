...
// 
Функция РезультатЗапросаВСоответствие(знач РезультатЗапроса, знач ПолеКлюча, знач УдалитьТипNULL = Истина) Экспорт
	ШаблонЭлемента = ПолучитьШаблонЭлементаРезультатаЗапроса(РезультатЗапроса, УдалитьТипNULL);
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеРезультат.Вставить(Выборка[ПолеКлюча], ЭлементМассива);
	КонецЦикла;
