...
Функция ПолучитьСписокДанныхВыбора(Настройки) Экспорт
	...
	Если Настройки.ПоискПоНаименованию И НЕ ПустаяСтрока(СтрокаПоиска) Тогда
		Если Параметры.Свойство("СпособПоискаСтроки")
			И Параметры.СпособПоискаСтроки = СпособПоискаСтрокиПриВводеПоСтроке.Начало Тогда
			
			ДлинаСтроки = Формат(СтрДлина(СтрокаПоиска), "ЧГ=");
			
			Отборы.Добавить("Т.Наименование >= &СтрокаПоиска");
			Отборы.Добавить(СтрШаблон("ПОДСТРОКА(Т.Наименование, 1, %1) = &СтрокаПоиска", ДлинаСтроки));
	...
	ПоляВыбора.Добавить("Т.Наименование КАК Наименование");
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеВыбора.Добавить(Выборка.Ссылка,
			...
			?(Выборка.ЭтоГруппа, КартинкаГруппы, Неопределено));
	КонецЦикла;	
...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	// добавим соединение с регистром доступных контрагентов, для ограничения доступности
	// запрос получения данных выполняется с правилом РАЗЛИЧНЫЕ, поэтому нет нужды переживать
	// о возможных дублях в результате соединений, однако не стоит учитывать влияение на производительность
	...
	// добавим поле замены для отбора по полю Валюта из основного договора
	Настройки.ЗаменаПолей.Вставить("Валюта", "ОсновнойДоговор.Валюта");
