...
Функция СформироватьПредставлениеПоКоллекции(знач НаборДанных, знач МассивПолей, знач Параметры) 
	...
	НетПрефиксов	= НЕ ЭтоСоответствие(ТипЗнч(Параметры.Префиксы)); //(ДопустимыеТипы.Найти(ТипЗнч(Параметры.Префиксы)) = Неопределено);
	НетФорматов		= НЕ ЭтоСоответствие(ТипЗнч(Параметры.Форматы)); //(ДопустимыеТипы.Найти(ТипЗнч(Параметры.Форматы)) = Неопределено);
