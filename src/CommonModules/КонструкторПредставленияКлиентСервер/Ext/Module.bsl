...
Функция ПодготовитьПредставление(знач Данные, знач Реквизиты, знач Настройки = Неопределено) Экспорт
	...
	Если ТипДанные = Тип("Массив") ИЛИ ТипДанные = Тип("ФиксированныйМассив") Тогда
		...
		Для Индекс = 0 По ПоследнийИндекс Цикл
			...
			СтрПредставление = СформироватьПредставлениеПоСтруктуре(НаборДанных, КолонкиСтроки, Параметры);
			Если ЗначениеЗаполнено(СтрПредставление) Тогда
				Представления.Добавить(СтрПредставление);
			КонецЕсли;
		КонецЦикла;
	ИначеЕсли ТипДанные = Тип("Структура") ИЛИ ТипДанные = Тип("ФиксированнаяСтруктура") Тогда
		СтрПредставление = СформироватьПредставлениеПоСтруктуре(Данные, Колонки, Параметры);
		Если ЗначениеЗаполнено(СтрПредставление) Тогда
			Представления.Добавить(СтрПредставление);
		КонецЕсли;
	КонецЕсли;
...
//
// Параметры:
//	НаборДанных	- Структура - имя и значение поля для представления
...
//   Строка
//
Функция СформироватьПредставлениеПоСтруктуре(знач НаборДанных, знач МассивПолей, знач Параметры) 
