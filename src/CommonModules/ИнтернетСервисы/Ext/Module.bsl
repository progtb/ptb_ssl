...
// Возвращает ошибку 500 с текстом, что интернет-сервисы отключены
//
// Возвращаемое значение:
//   HTTPСервисОтвет
//
...
Функция ПолучитьОтвет400(ТекстОшибки, Отказ = Ложь) Экспорт
	...
	Ответ.УстановитьТелоИзСтроки(ПолучитьСтрокуJSON(СтруктураОшибки));
...
// Устарела. Рекомендуется использовать ИнтернетСервисыКлиентСервер.СоздатьHTTPЗапрос
...
// Устарела. Рекомендуется использовать ИнтернетСервисыКлиентСервер.СоздатьHTTPСоединение
