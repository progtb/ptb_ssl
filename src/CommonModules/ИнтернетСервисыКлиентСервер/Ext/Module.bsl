...
// Возвращает признак, что переданные данные, являются коллекцией error
// Коллекцией error может быть Структура или Соответствие с ключом type = error
//
// Параметры:
//	Коллекция	- Произвольный - данные для проверки, что это коллекция error
//
// Возвращаемое значение:
//   Булево
//
Функция ЭтоКоллецияError(знач Коллекция) Экспорт
	Возврат (ТипЗнч(Коллекция) = Тип("Структура") И Коллекция.Свойство("type") И Коллекция.type = "error")
		ИЛИ (ТипЗнч(Коллекция) = Тип("Соответствие") И Коллекция.Получить("type") = "error");
КонецФункции

// Возвращает структуру параметров для подключения
//
// Параметры:
//	АдресСервера	- Строка
...
//
Функция ПараметрыПодключения(знач АдресСервера, знач ИмяПользователя, знач Пароль = "") Экспорт
	СтруктураАдреса = ОбщегоНазначенияКлиентСервер.СтруктураURI(АдресСервера);
	
	Возврат Новый Структура("АдресСервера,Пользователь,Пароль,СтруктураАдреса",
		АдресСервера,
		ИмяПользователя,
		Пароль,
		СтруктураАдреса);
...
//
// Параметры:
//	Подключение		- Структура - см. atc_ИнтернетPСервисыКлиентСервер.ПараметрыПодключения
...
//	HTTPЗапрос
//
Функция СоздатьHTTPЗапрос(знач Подключение, знач АдресРесурса, знач Параметры = Неопределено, знач Заголовки = Неопределено) Экспорт
	ПутьНаСервере = Подключение.СтруктураАдреса.ПутьНаСервере;
...
//	HTTPСоединение
//
Функция СоздатьHTTPСоединение(знач Подключение, знач Таймаут = Неопределено) Экспорт
	ИмяСервера	= Подключение.СтруктураАдреса.ИмяСервера;
	
	Если Подключение.СтруктураАдреса.Схема = "https" Тогда
...
Функция ЗначениеВСтрокуJSON(знач ОбъектДанных, знач Настройки = Неопределено, знач Параметры = Неопределено) Экспорт
	...
	#Если ВебКлиент Тогда
	    Результат = ИнтернетСервисыВызовСервера.ЗначениеВСтрокуJSON(ОбъектДанных, Настройки, Параметры);
	#Иначе
...
Функция ЗначениеИзСтрокиJSON(знач СтрокаJSON,
	...
	#Если ВебКлиент Тогда
		Значение = ИнтернетСервисыВызовСервера.ЗначениеИзСтрокиJSON(СтрокаJSON, КакСоответствие, СвойстваДаты, ФорматДаты);
	#Иначе
