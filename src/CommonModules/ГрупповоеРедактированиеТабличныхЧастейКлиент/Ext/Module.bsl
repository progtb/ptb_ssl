...
Процедура ОткрытьФормуЗаполненияВыделенныхСтрок(Форма) Экспорт
	...
	Если ТипЗнч(КоллекцияДанныхФормы) = Тип("ДанныеФормыКоллекция") ИЛИ ТипЗнч(КоллекцияДанныхФормы) = Тип("ДанныеФормыДерево")Тогда
		...
		Для каждого КлючИЗНачение Из ДоступныеКолонкиСоответствие Цикл
            ...
            СтруктураКолонки.Вставить("ПараметрыВыбора"			, ПолучитьПараметрыВыбораДляПередачиНаСервер(ЭлементКолонка));
			СтруктураКолонки.Вставить("СвязиПараметровВыбора"	, ПолучитьСвязиПараметровВыбораДляПередачиНаСервер(ЭлементКолонка));
...
Процедура РасчетСуммыВыделенныхСтрок(Форма) Экспорт
	...
	Если НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "_Служебный_ОписаниеТаблицДляГрупповогоРедактирования_") Тогда
		Возврат;
	КонецЕсли;
...
Функция ПолучитьПараметрыВыбораДляПередачиНаСервер(ЭлементФормы) 
...
Функция ПолучитьСвязиПараметровВыбораДляПередачиНаСервер(ЭлементФормы) 
	...
	Для Каждого ЭлементСвязи Из СвязиПараметровВыбора Цикл
		Структура = Новый Структура("Имя, ПутьКДанным");			
		ЗаполнитьЗначенияСвойств(Структура, ЭлементСвязи);
		Массив.Добавить(Структура);
	КонецЦикла; 
