...
Функция ТаблицаПолейПредставления() Экспорт
	...
	ТаблицаЗначений.Колонки.Добавить("Префиксы"			, Новый ОписаниеТипов());
	ТаблицаЗначений.Колонки.Добавить("Форматы"			, Новый ОписаниеТипов());
...
Процедура ЗаполнитьДеревоПараметровРекурсивно(знач СтрокаРодитель, знач Данные)
	...
	Для Каждого КлючИЗначение Из ИсточникДанных Цикл
		...
		СтрокаДерева.ИмяРеквизита	= КлючИЗначение.Ключ;
		СтрокаДерева.ПолныйПуть		= ?(СтрокаРодитель.ЭтоРаздел, "", СтрокаРодитель.ПолныйПуть + ".") + КлючИЗначение.Ключ;
		СтрокаДерева.ТипДанных		= ТипДанных;
		СтрокаДерева.ЭтоРаздел		= Ложь;
		СтрокаДерева.ЭтоГруппа		= НЕ ЭтоПростойТип;
		СтрокаДерева.КартинкаТипа	= ОбщегоНазначенияУМТРКлиентСервер.НомерКартинкиПоТипуДанных(ТипДанных);
...
Функция ПолучитьПоляПредставленияОбласти(знач НаборПредставлений, знач ИмяОбласти, знач ПараметрыМакета)
	...
	Для Каждого КлючИЗначение Из НаборПредставлений Цикл
		...
		Префиксы			= ОбщегоНазначенияПТБКлиентСервер.СвойствоСтруктуры(КлючИЗначение.Значение, "Префиксы", Неопределено);
		Форматы				= ОбщегоНазначенияПТБКлиентСервер.СвойствоСтруктуры(КлючИЗначение.Значение, "Форматы", Неопределено);
		Предопределенный	= ОбщегоНазначенияПТБКлиентСервер.СвойствоСтруктуры(КлючИЗначение.Значение, "Предопределенный", Ложь);
		
		ИсходныеДанные = Новый Структура("Реквизиты,Префиксы,Форматы", Реквизиты, Префиксы, Форматы);
		...
		СтрокаТаблицы.Префиксы			= ОбщегоНазначенияПТБКлиентСервер.СвойствоСтруктуры(ДанныеПредставления, "Префиксы", Неопределено);
		СтрокаТаблицы.Форматы			= ОбщегоНазначенияПТБКлиентСервер.СвойствоСтруктуры(ДанныеПредставления, "Форматы", Неопределено);
