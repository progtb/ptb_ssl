...
Процедура ЗаполнитьДеревоПоказателямиНаСервере(знач Показатели)
	Если НЕ ТипЗнч(Показатели) = Тип("Массив") Тогда
		Возврат;
	КонецЕсли;

	Для Каждого ГруппаПоказателей Из Показатели Цикл
		...
		СтрокаГруппы.Показатель		= НСтр("ru='Группа_Переменные'");
		СтрокаГруппы.ЭтоГруппа		= Истина;
		СтрокаГруппы.Описание		= ГруппаПоказателей.Описание; 
		
		Для Каждого ЭлементПоказателя Из ГруппаПоказателей.Элементы Цикл
