...
Процедура СообщитьКонтрольРегистрОстатки(ДополнительныеСвойства, Отказ) Экспорт
	...
	РеквизитыШапки = ДополнительныеСвойства.ПараметрыПроведения.Реквизиты;
	ДанныеРегистра = ДополнительныеСвойства.ПараметрыПроведения.РегистрОстатки;
	ДанныеРегистра.Индексы.Добавить("Пользователь");
	
	Для Каждого СтрокаКонтроля Из РезультатКонтроля Цикл
		СтруктураОтбора = Новый Структура("Пользователь");
		ЗаполнитьЗначенияСвойств(СтруктураОтбора, СтрокаКонтроля);
		
		МассивСтрок = ДанныеРегистра.НайтиСтроки(СтруктураОтбора);
		НомерСтроки = ?(МассивСтрок.Количество() = 0, 0, МассивСтрок[0].НомерСтрокиТЧ);
		...
		Если СтрокаКонтроля.Количество < 0 Тогда
			...
			ПутьКРеквизиту = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ТабличнаяЧасть", НомерСтроки, "Количество");
				
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, РеквизитыШапки.Ссылка, ПутьКРеквизиту, "Объект", Отказ);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
