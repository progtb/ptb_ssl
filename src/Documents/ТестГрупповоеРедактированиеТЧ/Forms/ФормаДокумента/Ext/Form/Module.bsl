...
&НаКлиенте
Процедура Подключаемый_ОбработчикиПриГрупповомРедактированииОкончание(Результат, ДопПараметры) Экспорт 
	
	//Если нрег(ДопПараметры.Колонка) = нрег("ПодразделениеКуратор") Тогда
	//	
	//	КураторПодразделения = ПолучитьРуководителяПодразделенияНаСервере(ДопПараметры.ЗначениеДляЗаполнения);
	//	
	//	КоллекцияДанныхФормы = ГрупповоеРедактированиеТабличныхЧастейКлиентСервер.ПолучитьКоллекциюДанныхФормыПоПутиКДанным(ЭтотОбъект, ДопПараметры.ПутьКДаннымТекущейТаблицы);
	//	
	//	Для Каждого ЭлементМассива Из ДопПараметры.МассивВыделенныхСтрок Цикл
	//		СтрокаТаблицы = КоллекцияДанныхФормы.НайтиПоИдентификатору(ЭлементМассива);

	//		СтрокаТаблицы.Куратор = КураторПодразделения;
	//   	КонецЦикла;
	//КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "ГрупповоеРедактированиеСтрокТаблицы" Тогда 
		...
		ОповещениеПослеИзменений = Новый ОписаниеОповещения("Подключаемый_ОбработчикиПриГрупповомРедактированииОкончание", ЭтотОбъект, Параметр);
		
		ГрупповоеРедактированиеТабличныхЧастейКлиент.ОбработкаОповещения(ЭтотОбъект, Параметр, Неопределено, ОповещениеПослеИзменений);	
	КонецЕсли;
КонецПроцедуры
