...
Процедура ЗаполнитьКонтактныеДанныеПользователя(ФабрикаКонтакты, ПространствоИмен, Отправитель)
	//Проверим наличие подсистемы бсп???
	ИспользуетсяБСП = (Метаданные.Справочники["Пользователи"].ТабличныеЧасти.Найти("КонтактнаяИнформация") <> Неопределено);
	Если ИспользуетсяБСП Тогда
		Массив = Новый Массив();
		Массив.Добавить(Отправитель);
		ТаблицаКонтактов = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(Массив);
	...
	//Иначе
	//	ФабрикаКонтакт = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ПространствоИмен, "Контакт"));
	//	ФабрикаКонтакт.ЗаписьКонтактнойИнформации = "";
	//	ФабрикаКонтакты.Контакт.Добавить(ФабрикаКонтакт);
	КонецЕсли; 
