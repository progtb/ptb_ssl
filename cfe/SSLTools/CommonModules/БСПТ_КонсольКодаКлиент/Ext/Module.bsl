...
Функция ЭтаКонсоль(знач Форма, знач ИмяРеквизита) Экспорт
	...
	ПараметрыФормы	= БСПТ_КонсольКодаКлиентСервер.ПолучитьПараметрыФормыКонсоли(Форма);
	ОбъектКонсоли	= Форма.Элементы[ПараметрыКонсоли.ИмяПоляHTML].Документ.defaultView;
	
	Структура = БСПТ_КонсольКодаКлиентСервер.СтруктураОбъектКонсоли();
...
Процедура ОповещатьПриИзменении(знач Консоль, знач Значение) Экспорт 
	...
	Если НЕ БСПТ_КонсольКодаКлиентСервер.ЭтоСтруктураОбъектКонсоли(Консоль) Тогда
		Возврат;
	КонецЕсли;
...
Функция ЗагрузитьПользовательскиеОбъекты(знач Консоль, знач Данные) Экспорт
	...
	СтрокаJSON = БСПТ_КонсольКодаКлиентСервер.СтруктуруВСтрокуJSON(СтруктураОбъекты);
...
Асинх Функция ИнициализацияКонсолиАсинх(знач Форма)
	...
	РасширениеПодключено = Ждать БСПТ_ФайловаяСистемаКлиентАсинх.ПодключитьРасширениеДляРаботыСФайлами();
	Если НЕ РасширениеПодключено Тогда
		БСПТ_ФайловаяСистемаКлиентАсинх.СообщитьОшибкуПодключенияРасширения(Ложь, Истина);
		
		Результат.ТекстОшибки = НСтр("ru='Расширение для работы с файлами не подключено.'");
		Возврат Результат;
	КонецЕсли;
	
	ИмяПараметры = БСПТ_КонсольКодаКлиентСервер.ИмяРеквизитаПараметрыКонсолиКода();
	...
	БСПТ_КонсольКодаКлиентСервер.УстановитьПараметрыФормыКонсоли(Форма, НовыеПараметры);
...
Асинх Функция ПроверитьИсходникиАсинх(знач Параметры)
	...
	Результат.ФайлВерсии = БСПТ_КонсольКодаКлиентСервер.ПолучитьПутьКФайлуВерсииКонсоли(Результат.КаталогИсходников, Параметры.Версия + ".ver");
	...
	Если ПустаяСтрока(Результат.АдресМакетаКонсоли) ИЛИ НЕ ЭтоАдресВременногоХранилища(Результат.АдресМакетаКонсоли) Тогда
		...
		БСПТ_КонсольКодаВызовСервера.ПолучитьМакетКонсоли(Результат.АдресМакетаКонсоли);
	КонецЕсли;
...
Процедура ОткрытьКонструкторЗапроса(знач Консоль, знач Текст, знач ДопПараметры)
	...
	Оповещение = Новый ОписаниеОповещения("КонструкторЗапросаЗавершение", БСПТ_КонсольКодаКлиент, ПараметрыОповещения);
...
Функция ЗагрузитьОбъектМетаданныхПоЗапросу(знач Консоль, знач ПараметрыЗапроса)
	...
	ДанныеОписания = БСПТ_КонсольКодаВызовСервера.ПолучитьОписаниеОбъектаМетаданных(ПараметрыЗапроса);
...
Функция ЗагрузитьСписокМетаданныхПоЗапросу(знач Консоль, знач ПараметрыЗапроса)
	...
	ДанныеОписания = БСПТ_КонсольКодаВызовСервера.ПолучитьСписокОбъектовМетаданных(ПараметрыЗапроса);
...
Асинх Процедура ЗагрузитьГлобальныеМодули(знач Консоль)
	...
	Если ПустаяСтрока(КаталогКода)Тогда
		Возврат;
	ИначеЕсли НЕ БСПТ_КонсольКодаКлиентСервер.ЭтоМассив(ПараметрыФормы.ГлобальныйМодули) Тогда
		Возврат;
	КонецЕсли;
...
Асинх Функция ЗагрузитьОбщийМодульПоИмениАсинх(знач Консоль, знач ИмяМодуля, знач Глобальный)
	...
	ИначеЕсли НЕ БСПТ_КонсольКодаКлиентСервер.ЭтоСоответствие(ПараметрыФормы.ИменаОбщихМодулей) Тогда
		Возврат ЗагруженоФункций;
	КонецЕсли;
...
Асинх Функция ЗагрузитьМодульМенеджераИлиОбъектаПоИмениАсинх(знач Консоль, знач СтруктураВыгрузки)
	...
	Если ФайлСуществует Тогда
		...
		ИмяКоллекции = БСПТ_КонсольКодаКлиентСервер.ПолучитьИмяКоллекцииМетаданныхПоТипу(СтруктураВыгрузки.ТипМетаданных);
...
Асинх Функция ЗагрузитьСтруктуруМодуляМенеджераИлиОбъектаПоЗапросуАсинх(знач Консоль, знач ЧастиЗапроса)
	...
	Если ЧастиЗапроса.Количество() = КоличествоЧастейЗапроса Тогда
		...
		БСПТ_КонсольКодаВызовСервера.ДополнитьСтруктуруВыгрузкиИсходниковМетаданных(СтруктураВыгрузки, ПолучитьРазделительПутиКлиента());
