...
Процедура ПриСозданииНаСервере(знач Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	ПараметрыКонсоли = БСПТ_КонсольКодаКлиентСервер.ПараметрыФормыКонсоли(Форма);
	
	ИнициализацияМетаданных(ПараметрыКонсоли);
	
	ИмяРеквизита = БСПТ_КонсольКодаКлиентСервер.ИмяРеквизитаПараметрыКонсолиКода();
...
Процедура ИнициализацияКоманднойПанели(знач Форма, знач ИмяРеквизита, знач Элемент = Неопределено, знач ПараметрыПанели = Неопределено) Экспорт
	...
	Если НЕ БСПТ_КонсольКодаКлиентСервер.ЭтоПараметрыПанелиКонсоли(ПараметрыПанели) Тогда
		ПараметрыПанели = БСПТ_КонсольКодаКлиентСервер.ПараметрыПанелиКонсоли();
	КонецЕсли;
	
	СтрШаблонИмени = "%1_%2";
	
	ПараметрыКонсоли = БСПТ_КонсольКодаКлиентСервер.ПолучитьСтрокуПараметровКонсоли(Форма, ИмяРеквизита);
	...
	КомандаФормы = Форма.Команды.Добавить(ИмяКоманды);
	КомандаФормы.Действие		= "КомандаКонсолиКода";
	КомандаФормы.Картинка		= БиблиотекаКартинок.БСПТ_ПробелыИТабуляция;
	КомандаФормы.Отображение	= ОтображениеКнопки.Картинка;
	...
	КомандаФормы = Форма.Команды.Добавить(ИмяКоманды);
	КомандаФормы.Действие		= "КомандаКонсолиКода";
	КомандаФормы.Картинка		= БиблиотекаКартинок.БСПТ_РежимСравнения;
	КомандаФормы.Отображение	= ОтображениеКнопки.Картинка;
...
Процедура ДобавитьРеквизитыКонсолиКода(знач Форма)
	РеквизитПараметры = БСПТ_КонсольКодаКлиентСервер.ИмяРеквизитаПараметрыКонсолиКода();
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, РеквизитПараметры) Тогда
		Возврат;
	КонецЕсли;
	
	РеквизитТаблица		= БСПТ_КонсольКодаКлиентСервер.ИмяТаблицыПараметрыКонсолейКода();
	РеквизитВремФайлы	= БСПТ_КонсольКодаКлиентСервер.ИмяРеквизитаВременныеФайлы();
...
Процедура ИнициализацияМетаданных(знач Параметры)
	...
	КоллекцияОбщихМодулей = БСПТ_КонсольКодаВызовСервера.ПолучитьКоллекциюОбщихМодулей();
	
	ОбщиеМодулиJSON			= БСПТ_КонсольКодаКлиентСервер.СтруктуруВСтрокуJSON(КоллекцияОбщихМодулей.Общие);
	КлиентскиеМодулиJSON	= БСПТ_КонсольКодаКлиентСервер.СтруктуруВСтрокуJSON(КоллекцияОбщихМодулей.Клиент);
	СерверныеМодулиJSON		= БСПТ_КонсольКодаКлиентСервер.СтруктуруВСтрокуJSON(КоллекцияОбщихМодулей.Сервер);
...
Функция ТаблицаПараметровКонсоли()
	...
	ТаблицаКонсоли.Колонки.Добавить("ИмяДляСравнения"		, ОбщегоНазначения.ОписаниеТипаСтрока(0));
	ТаблицаКонсоли.Колонки.Добавить("ИмяРеквизита"			, ОбщегоНазначения.ОписаниеТипаСтрока(0));
	ТаблицаКонсоли.Колонки.Добавить("ИмяКоманднойПанели"	, ОбщегоНазначения.ОписаниеТипаСтрока(0));
	ТаблицаКонсоли.Колонки.Добавить("ИмяПоляHTML"			, ОбщегоНазначения.ОписаниеТипаСтрока(0));
	ТаблицаКонсоли.Колонки.Добавить("ИндексныйФайл"			, ОбщегоНазначения.ОписаниеТипаСтрока(0));
	...
	ТаблицаКонсоли.Колонки.Добавить("КаталогИсходногоКода"	, ОбщегоНазначения.ОписаниеТипаСтрока(0));
	...
	ТаблицаКонсоли.Колонки.Добавить("ТекстАлгоритма"		, ОбщегоНазначения.ОписаниеТипаСтрока(0));
	ТаблицаКонсоли.Колонки.Добавить("ОригинальныйТекст"		, ОбщегоНазначения.ОписаниеТипаСтрока(0));
	
	ТаблицаКонсоли.Колонки.Добавить("ТекущаяТема"			, ОбщегоНазначения.ОписаниеТипаСтрока(30));
	...
	ТаблицаКонсоли.Колонки.Добавить("ДирективаКомпиляции"	, ОбщегоНазначения.ОписаниеТипаСтрока(10)); // НаКлиенте, НаСервере, пустое значение
