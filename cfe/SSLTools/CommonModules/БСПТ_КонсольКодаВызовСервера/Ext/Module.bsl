...
Процедура ПолучитьМакетКонсоли(знач АдресМакета) Экспорт
	...
	ДанныеИсходников = Обработки.БСПТ_КонсольКода.ПолучитьМакет("src");
...
Процедура ДополнитьСтруктуруВыгрузкиИсходниковМетаданных(СтруктураВыгрузки, знач РазделительПути) Экспорт
	...
	Если Имена.Количество() = ЧастейМетаданных Тогда
		...
		КаталогТипа = БСПТ_КонсольКодаКлиентСервер.ПолучитьИмяКаталогаВыгрузкиМетаданныхПоТипу(СтруктураВыгрузки.ТипМетаданных);
...
Функция ПолучитьОписаниеОбъектаМетаданных(знач ТипОбъекта) Экспорт
	
	Результат = Новый Структура("Описание,Адрес", "", "");
	
	Части = СтрРазделить(ТипОбъекта, ".");
	
	ИмяКоллекции = БСПТ_КонсольКодаКлиентСервер.ПолучитьИмяКоллекцииМетаданныхПоТипу(Части[0]);
	...
	Если ЗначениеЗаполнено(ИмяКоллекции) Тогда
		...
		Результат.Адрес		= ИмяКоллекции + ".items." + ОбъектМетаданных.Имя;
		Результат.Описание	= БСПТ_КонсольКодаКлиентСервер.СтруктуруВСтрокуJSON(СтруктураОбъекта);;
...
Функция ПолучитьСписокОбъектовМетаданных(знач ТипОбъектов) Экспорт
	...
	ИмяКоллекции = БСПТ_КонсольКодаКлиентСервер.ПолучитьИмяКоллекцииМетаданныхПоТипу(ТипОбъектов);
	...
	Если ЗначениеЗаполнено(ИмяКоллекции) Тогда
		...
		Результат.Описание	= БСПТ_КонсольКодаКлиентСервер.СтруктуруВСтрокуJSON(СписокОбъектов);
...
Функция ПолучитьФайлыМакетаКонсоли(знач АдресМакета, знач КаталогНаКлиенте, знач ИдентификаторФормы, знач Версия) Экспорт
	...
	КаталогФайлов = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(КаталогВременныхФайлов(), "bsl_temp");
	...
	ФайлВерсии = БСПТ_КонсольКодаКлиентСервер.ПолучитьПутьКФайлуВерсииКонсоли(КаталогФайлов, Версия);
...
Функция СтруктуруВСтрокуJSON(знач Структура) Экспорт
	...
	Возврат БСПТ_КонсольКодаКлиентСервер.СтруктуруВСтрокуJSON(Структура);
